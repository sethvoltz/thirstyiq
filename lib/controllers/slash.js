...
function runRemove(environment, command, params) {
  ...
  if (!groupName) {
    return 'Please specify a group name to remove. Use `/thirsty list` to get a list of groups';
  }

  return group.delete(environment.team_id, environment.user_id, groupName)
  .then(() => `Successfully removed group '${groupName}'.`)
  .catch(function (error) {
    if (error && error.code && error.code === 'ConditionalCheckFailedException') {
      return `Either the group '${groupName}' does not exist, or you do not own the group.`;
    }
    throw error;
  });
}
...
function helpRemove() {
  return 'Remove a group you created.\n'
    + 'Usage: `/thirsty remove <group name>\n'
    + 'Tips:\n'
    + '   Group names may contain spaces if you "surround it with quotes".';
}
...
function formatGroupList(list) {
  ...
  return list.map(item => [item.name, item.private ? '(private)' : ''].join(' ').trim()).join(', ')
}
