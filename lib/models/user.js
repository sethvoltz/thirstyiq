...
User.prototype.canSeeGroup = function (userId, group) {
  if (!group) {
    var error = new Error('No group given');
    error.code = 'NoGroupException';
    throw error;
  }
  if (!group.private || group.creator === userId || group.members.indexOf(userId) !== -1) {
    return group;
  } else {
    var error = new Error('User not able to see group');
    error.code = 'GroupNotVisibleException';
    throw error;
  }
}
